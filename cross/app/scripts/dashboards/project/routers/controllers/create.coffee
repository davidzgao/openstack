...
angular.module('Cross.project.routers')
  .controller 'project.routers.RouterCreateCtr', ($scope, $http,
  ...
  .controller 'project.routers.routerActionCtr', ($scope) ->
    return
  .controller 'project.routers.AddGateway', ($scope, $http, $window,
  $state, $q, $stateParams) ->
    serverURL = $window.$CROSS.settings.serverURL
  ...
  .controller 'project.routers.AddPort', ($scope, $http, $window,
  $state, $q, $stateParams) ->
    serverURL = $window.$CROSS.settings.serverURL
...
class RouterCreateModal extends $cross.Modal
  title: 'Create Router'
  slug: 'router_create'
  single: true
...
class AddGatewayModal extends $cross.Modal
  title: 'Add Gateway'
  slug: 'add_gateway'
  single: true
...
class AddPortModal extends $cross.Modal
  title: _ 'Add Port'
  slug: 'add_port'
  single: true
  modalLoading: true
    ...
    if form.ip
      ...
      $http.put addInterfaceURL, param
        ...
        .error (error) ->
          if error
            if error.status == 409
              toastr.error _("The IP address has been used! Failed to add interface.")
            else
              toastr.error _("Failed to add interface.")
          else
            toastr.error _("Failed to add interface.")
