...
angular.module('Cross.project.security_group')
  .controller 'project.security_group.SecurityGroupDetailCtr', ($scope, $http, $window,
    ...
    $scope.note =
      ...
      title:
        ip: _("Must be cidr.")
        port: _("Must be an integer between 1 and 65535.")
      action:
        delete: _("Delete")
        add: _("Add")
        save: _("Save")
    ...
    getRules = (callback) ->
      ...
      $scope.mode = if $CROSS.settings.use_neutron then 'in-out' else 'default'
      $cross.networks.securityGroupGet $http, $scope.currentId, (err, data) ->
        if not err
    ...
    $scope.saveAddRule = ->
      ...
      if protocol == 'ICMP'
        ...
        if !is_port_valid(port, 65535)
            valid = false
            $scope.rule_port_invalid = 'invalid'
            $scope.rule_port_placeholder = _('Must be an integer between 1 and 65535.')
            $scope.rule_val.port = ''
      ...
      data =
        ...
        tenant_id: tenantId
      $cross.networks.securityGroupAddRule $http, data, (err, data) ->
        if not err
    ...
    $scope.deleteRule = (item) ->
      $cross.networks.securityGroupRuleDelete $http, item.id, (err, data) ->
        if not err
