...
preInitial =
  ...
  checkBrowser: ->
    browserAllowed = $CROSS.settings.allowBrowsers
    current = preInitial.browser()
    brCheck = document.getElementById("_cross_browser_check")
    ...
    title = children[0]
    browsers = children[1]
    locale = $CROSS.settings.locale || 'en'
    titleMsg =
    ...
    loading = document.getElementById("_cross_pre_loading")
    app = document.getElementById("_cross_app_main")
