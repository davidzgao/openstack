...
var Client = Class.extend({
  ...
  url_for: function (endpoint_type, service_type, region_name) {
    ...
    for (var i = 0; i < this.service_catalog.length; i++) {
      if (this.service_catalog[i].type === search_service_type) {
        ...
        var url = null;
        if(region_name) {
          for(var j = 0; j < this.service_catalog[i].endpoints.length; j++) {
            if(this.service_catalog[i].endpoints[j]['region'] === region_name) {
              url = this.service_catalog[i].endpoints[j][endpoint_type];
              break;
            }
          }
        } else {
          url = this.service_catalog[i].endpoints[0][endpoint_type];
        }
  ...
  process_response: function (method, url, data, status, response_text, req_headers, resp_headers, params, callback) {
    ...
    if (status === 0 || status >= 400) {
      ...
      try {
        ...
        if (api_error.hasOwnProperty(error_key)) api_error = api_error[error_key];
  ...
  request: function (params, callback) {
    ...
    if (params.query) {
      ...
      Object.keys(params.query).forEach(function (key) {
        var param_val = params.query[key];
        if (param_val instanceof Array) {
          for (var val = 0; val < param_val.length; val ++) {
            query_params.push(key + "=" + param_val[val])
          }
        } else {
    ...
    if (params.data === undefined || !Object.keys(params.data).length) {
  ...
  authenticate: function (params, callback) {
    ...
    if (params.project) {
      ...
      credentials.auth.tenantId = params.project_id
    }
...
var Manager = Class.extend({
  ...
  init: function (client) {
    ...
    this.error_key = this.error_key || "error"
  ...
  get_base_url: function (params) {
    ...
    if(params.version) {
      return urljoin(base, params.version || '', this.prepare_namespace(params));
    } else {
  ...
  _downloadBinaryStream: function (params, headers, token) {
    var client = this.client;
