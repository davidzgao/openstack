...
angular.module('Unicorn.dashboard.pricelist')
  .controller 'dashboard.pricelist.InstanceDetailCtr', ($scope, $http, $window, $q,$state, $stateParams, $location ) ->
    ...
    $scope.columnDefs = [
      {
        ...
        cellTemplate: '<div class="ngCellText"><a href="#/dashboard/pricelist/instanceDetail?insName={{item[col.field]}}&">{{item[col.field]}}</a></div>'
      }
      {
    ...
    $http.get("#{serverUrl}/prices")
      .success (itemList) ->
        for event in itemList
          resourcePriceObj[event.name] = event.price
...
class InstanceDetailModal extends $unicorn.Modal
  ...
  handle: ($scope, options)->
    $state = options.$state
    toastr.success "Post creating pricelist successful"
    options.callback false
    $state.go 'dashboard.pricelist', {}, {reload: false}
