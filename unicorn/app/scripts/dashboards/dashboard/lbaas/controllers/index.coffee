...
angular.module('Unicorn.dashboard.lbaas')
  .controller 'dashboard.lbaas.LBCtr', ($scope, $http, $window,
  $q, $state) ->
...
class LBTable extends $unicorn.TableView
  labileStatus: []
  slug: 'lbs'
  columnDefs: [
    {
      field: "name"
      displayName: _("Name")
      cellTemplate: '<div class="ngCellText enableClick"><a ui-sref="dashboard.lbaas.lbId.overview({lbId:item.id})">{{item.name}}</a></div>'
    }
    {
      field: "subnet"
  ...
  initialAction: ($scope, options) ->
    ...
    serverURL = $UNICORN.settings.serverURL
    $scope.create = () ->
      $state.go "dashboard.lbaas.create"
    $scope.vipManage = () ->
      if $scope.selectedItems.length == 1
        $state.go 'dashboard.lbaas.LBId.vip', {LBId: $scope.selectedItems[0].id}
      else
        return
    $scope.memberManage = () ->
      if $scope.selectedItems.length == 1
        $state.go 'dashboard.lbaas.LBId.member', {LBId: $scope.selectedItems[0].id}
      else
        return
      ...
      deletePool = (poolId, poolName) ->
        $http.delete "#{serverURL}/lb/pools/#{poolId}"
          .success (data) ->
            ...
            $state.go "dashboard.lbaas", {}, {reload: true}
          .error (err) ->
