...
angular.module('Unicorn.dashboard.instance')
  .controller 'dashboard.instance.InstanceMonitorCtr', ($scope) ->
    $scope.monitorTabs = [{
        title: _('Real Time')
        enable: true
...
angular.module('Unicorn.dashboard.instance')
  .controller 'dashboard.instance.InstanceMonitorRealTimeCtl', ($scope,
  $http,$window, $q, $stateParams, $state, $gossip) ->
    ...
    meterURL = $window.$UNICORN.settings.serverURL + '/meters/'
    resourceURL = $window.$UNICORN.settings.serverURL + '/resources_per'
