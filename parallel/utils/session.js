...
function setName(req, name) {
  // get platform from headers or query,
  // if not exist, use unkown instead.
  var platform = req.headers['x-platform'];
  platform = platform || req.query['x-platform'];
  platform = platform || 'unkown';
  if(req.query['x-platform']) {
    delete req.query['x-platform'];
  }
